 <?php
  ...
  $store->update_manager_data(); 

  $id = $_GET['id'];
  $con = mysqli_connect("localhost","root","","store_member");
  $sql = mysqli_query($con,"SELECT `id`, `picture`, `username`, `password`, `lastname`, `firstname`, `middlename`, `number`, `email`, `address`, `status` FROM manager WHERE id = $id");
 ...
 <div id="box-1" style="background-color: #f0f0f0; padding-bottom: 160px;">
  ...
  <div class="col">
    ...
    <div class="but2">
    <center><button style="width: 200px;height: 50px; background-color: green;font-weight: bold;color:white; " class="but2" type="submit" name ="update_manager">Update</button>
    </div>
		</form>
